...
class Drawing:
    ...
    def health_bar(self):
        """Draw a health bar in the bottom-right corner of the screen."""
        bar_width = 200
        bar_height = 20
        x = WIDTH - bar_width - 20
        y = HEIGHT - bar_height - 20
        fill = int(bar_width * self.player.health / PLAYER_MAX_HEALTH)
        # background
        pygame.draw.rect(self.sc, (60, 0, 0), (x, y, bar_width, bar_height), border_radius=4)
        # fill
        if fill > 0:
            color = RED if self.player.health < 30 else (0, 200, 0)
            pygame.draw.rect(self.sc, color, (x, y, fill, bar_height), border_radius=4)
        # border
        pygame.draw.rect(self.sc, WHITE, (x, y, bar_width, bar_height), 2, border_radius=4)
        # text
        hp_text = self.font.render(f'HP: {self.player.health}', 0, WHITE)
        self.sc.blit(hp_text, (x, y - 34))
